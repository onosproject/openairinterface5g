...
//dlsch_ACK[1] = 1; //pucch_payload[0];

printf("ACKNACK process_HARQ_feedback fr/subfr %d %d dlsch_ACK[0] %d _eNB->ulsch_eNB[(uint8_t)UE_id]->harq_processes[harq_pid]->o_ACK[1] %d CC %d\n", frame, subframe, dlsch_ACK[0], phy_vars_eNB->ulsch_eNB[(uint8_t)UE_id]->harq_processes[harq_pid]->o_ACK[1], phy_vars_eNB->CC_id);
                      ...
                      int16_t *n1_pucch3)
{
  ...
  if (frame_parms->frame_type == FDD ) {
    ...
    AssertFatal(n_configured_SCCs < 2, "No support for more than 1 CC yet");
