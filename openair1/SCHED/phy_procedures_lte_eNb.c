  ...
  cudaEventDestroy(stopEvent);*/
	...
	VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PHY_ENB_ULSCH_MSG3,0);
	  ...
	  if (phy_vars_eNB->mac_enabled==1) {
        ...
        if (abstraction_flag == 0) {
#ifdef CUDA
           phy_vars_eNB->eNB_UE_stats[i].timing_advance_update = 1;
//           phy_vars_eNB->eNB_UE_stats[i].timing_advance_update = sync_pos - phy_vars_eNB->lte_frame_parms.nb_prefix_samples/4;
#else
