...
/* \brief Implementation of turbo decoder thread invoking
...
static void *isip_turbo_proc(void *param)
{
  static int isip_turbo_proc_status;

  // PHY_VARS_eNB *eNB = PHY_vars_eNB_g[0][0];
  isip_turbo *turbo_pram = (isip_turbo *)param;
  int ret = 0;
  ...
  // printf("[ISIP][SCHED][eNB] Turbo thread %d started on CPU %d \n", turbo_pram->thread_id, sched_getcpu());

  while (!oai_exit)
  {
    pthread_cond_wait(&turbo_pram->cond_rx, &turbo_pram->mutex_rx);

    int UE_id = turbo_pram->UE_id;
    int harq_pid = turbo_pram->harq_pid;
    int llr8_flag = turbo_pram->llr8_flag;
    int current_thread_num = turbo_pram->thread_id;
    PHY_VARS_eNB *eNB = turbo_pram->eNB;

    ret = isip_ulsch_decoding_data(eNB, UE_id, harq_pid, llr8_flag, current_thread_num);

    turbo_pram->ret = ret;
    turbo_pram->flag_done = 1 << current_thread_num;
  }
  isip_turbo_proc_status = 0;
  return &isip_turbo_proc_status;
}
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  // ISIP Create Thread Start
  for (i = 0; i < ISIP_TURBO_THREAD_NUM; ++i)
  {
    pthread_attr_init(&eNB->isip_turbo_thread[i].attr_turbo);
    pthread_mutex_init(&eNB->isip_turbo_thread[i].mutex_rx, NULL);
    pthread_cond_init(&eNB->isip_turbo_thread[i].cond_rx, NULL);

    eNB->isip_turbo_thread[i].thread_id = i;
    eNB->isip_turbo_thread[i].flag_wait = 1;

    pthread_create(&(eNB->isip_turbo_thread[i].pthread_rx), &eNB->isip_turbo_thread[i].attr_turbo, isip_turbo_proc, &eNB->isip_turbo_thread[i]);
    // printf("[ISIP][CREATE] Turbo thread %d \n", eNB->isip_turbo_thread[i].thread_id);
  }
  // ISIP Create Thread End
  ...
  for (ch_realization = 0; ch_realization < n_ch_rlz; ch_realization++)
  {
    ...
    */
    ...
    for (SNR = snr0; SNR < snr1; SNR += input_snr_step)
    {
      ...
      reset_meas(&eNB->isip_turbo_stats);
      reset_meas(&eNB->isip_wait_loop);

      int isip_c;
      for (isip_c = 0; isip_c < ISIP_TURBO_THREAD_NUM; isip_c++)
      {
        reset_meas(&eNB->isip_ulsch_turbo_decoding_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_init_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_alpha_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_beta_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_gamma_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_ext_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_intl1_stats[isip_c]);
        reset_meas(&eNB->ulsch_tc_intl2_stats[isip_c]);
      }
...
*/
          ...
          for (i = 0; i < eNB->frame_parms.samples_per_tti; i++)
          {
            for (aa = 0; aa < eNB->frame_parms.nb_antennas_rx; aa++)
            {
              ...
              ((short *)&eNB->common_vars.rxdata[0][aa][eNB->frame_parms.samples_per_tti * subframe])[2 * i + 1] = (short)((tx_gain * r_im[aa][i]) + (iqim * tx_gain * r_re[aa][i]) + sqrt(sigma2 / 2) * gaussdouble(0.0, 1.0));
            }
          }

          if (n_frames == 1)
          {
            printf("rx_level Null symbol %f\n", 10 * log10((double)signal_energy((int *)&eNB->common_vars.rxdata[0][0][(eNB->frame_parms.samples_per_tti << 1) - eNB->frame_parms.ofdm_symbol_size], OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES / 2)));
          ...
          SNRmeas = 10 * log10(((double)signal_energy((int *)&eNB->common_vars.rxdata[0][0][160 + (eNB->frame_parms.samples_per_tti * subframe)],
                                                      OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES / 2)) /
                                   ((double)signal_energy((int *)&eNB->common_vars.rxdata[0][0][(eNB->frame_parms.samples_per_tti << 1) - eNB->frame_parms.ofdm_symbol_size],
                                                          OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES / 2)) -
                               1) +
                    10 * log10(eNB->frame_parms.N_RB_UL / nb_rb);
      ...
      if (dump_perf == 1)
      {
        ...
        std_phy_proc_tx = sqrt((double)UE->phy_proc_tx.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / UE->phy_proc_tx.trials - pow((double)UE->phy_proc_tx.diff / UE->phy_proc_tx.trials / cpu_freq_GHz / 1000, 2));
        ...
        printf("|__ Statistics                         std: %f us max: %fus min: %fus median %fus q1 %fus q3 %fus n_dropped: %d packet \n", std_phy_proc_tx, t_tx_max, t_tx_min, tx_median, tx_q1, tx_q3,
               n_tx_dropped);
        std_phy_proc_tx_ifft = sqrt((double)UE->ofdm_mod_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / UE->ofdm_mod_stats.trials - pow((double)UE->ofdm_mod_stats.diff / UE->ofdm_mod_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        std_phy_proc_tx_mod = sqrt((double)UE->ulsch_modulation_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / UE->ulsch_modulation_stats.trials - pow((double)UE->ulsch_modulation_stats.diff / UE->ulsch_modulation_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        std_phy_proc_tx_enc = sqrt((double)UE->ulsch_encoding_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / UE->ulsch_encoding_stats.trials - pow((double)UE->ulsch_encoding_stats.diff / UE->ulsch_encoding_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        printf("|__ ULSCH turbo encoding time         :%f us (%d trials)\n",
               ((double)UE->ulsch_turbo_encoding_stats.trials / UE->ulsch_encoding_stats.trials) * (double)UE->ulsch_turbo_encoding_stats.diff / UE->ulsch_turbo_encoding_stats.trials / cpu_freq_GHz / 1000.0, UE->ulsch_turbo_encoding_stats.trials);
        printf("|__ ULSCH rate-matching time          :%f us (%d trials)\n",
               ((double)UE->ulsch_rate_matching_stats.trials / UE->ulsch_encoding_stats.trials) * (double)UE->ulsch_rate_matching_stats.diff / UE->ulsch_rate_matching_stats.trials / cpu_freq_GHz / 1000.0, UE->ulsch_rate_matching_stats.trials);
        printf("|__ ULSCH sub-block interleaving time :%f us (%d trials)\n",
               ((double)UE->ulsch_interleaving_stats.trials / UE->ulsch_encoding_stats.trials) * (double)UE->ulsch_interleaving_stats.diff / UE->ulsch_interleaving_stats.trials / cpu_freq_GHz / 1000.0, UE->ulsch_interleaving_stats.trials);
        printf("|__ ULSCH multiplexing time           :%f us (%d trials)\n",
               ((double)UE->ulsch_multiplexing_stats.trials / UE->ulsch_encoding_stats.trials) * (double)UE->ulsch_multiplexing_stats.diff / UE->ulsch_multiplexing_stats.trials / cpu_freq_GHz / 1000.0, UE->ulsch_multiplexing_stats.trials);

        printf("\n\neNB RX function statistics (per 1ms subframe)\n\n");
        std_phy_proc_rx = sqrt((double)eNB->phy_proc_rx.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / eNB->phy_proc_rx.trials - pow((double)eNB->phy_proc_rx.diff / eNB->phy_proc_rx.trials / cpu_freq_GHz / 1000, 2));
        ...
        printf("|__ Statistcs                           std: %fus max: %fus min: %fus median %fus q1 %fus q3 %fus n_dropped: %d packet \n", std_phy_proc_rx, t_rx_max, t_rx_min, rx_median, rx_q1, rx_q3,
               n_rx_dropped);
        std_phy_proc_rx_fft = sqrt((double)eNB->ofdm_demod_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / eNB->ofdm_demod_stats.trials - pow((double)eNB->ofdm_demod_stats.diff / eNB->ofdm_demod_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        std_phy_proc_rx_demod = sqrt((double)eNB->ulsch_demodulation_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / eNB->ulsch_demodulation_stats.trials - pow((double)eNB->ulsch_demodulation_stats.diff / eNB->ulsch_demodulation_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        std_phy_proc_rx_dec = sqrt((double)eNB->ulsch_decoding_stats.diff_square / pow(cpu_freq_GHz, 2) / pow(1000, 2) / eNB->ulsch_decoding_stats.trials - pow((double)eNB->ulsch_decoding_stats.diff / eNB->ulsch_decoding_stats.trials / cpu_freq_GHz / 1000, 2));
        ...
        printf("\n\n[ISIP] All turbo decoding time :%f us (%d trials, max %f us)\n\n",
               (double)eNB->isip_turbo_stats.diff / eNB->isip_turbo_stats.trials / cpu_freq_GHz / 1000.0,
               eNB->isip_turbo_stats.trials,
               (double)eNB->isip_turbo_stats.max / cpu_freq_GHz / 1000.0);

        for (isip_c = 0; isip_c < ISIP_TURBO_THREAD_NUM; isip_c++)
        {

          printf("[ISIP Thread %d] turbo decoding time :%f us (max: %f us)\n",
                 isip_c,
                 (double)eNB->isip_ulsch_turbo_decoding_stats[isip_c].diff / eNB->isip_ulsch_turbo_decoding_stats[isip_c].trials / cpu_freq_GHz / 1000.0,
                 (double)eNB->isip_ulsch_turbo_decoding_stats[isip_c].max / cpu_freq_GHz / 1000.0);

          printf("[ISIP Thread %d] turbo decoding init: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_init_stats[isip_c].diff / eNB->ulsch_tc_init_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding alpha: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_alpha_stats[isip_c].diff / eNB->ulsch_tc_alpha_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding beta: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_beta_stats[isip_c].diff / eNB->ulsch_tc_beta_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding gamma: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_gamma_stats[isip_c].diff / eNB->ulsch_tc_gamma_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding ext: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_ext_stats[isip_c].diff / eNB->ulsch_tc_ext_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding intl1: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_intl1_stats[isip_c].diff / eNB->ulsch_tc_intl1_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("[ISIP Thread %d] turbo decoding intl2: %f us \n",
                 isip_c,
                 (double)eNB->ulsch_tc_intl2_stats[isip_c].diff / eNB->ulsch_tc_intl2_stats[isip_c].trials / cpu_freq_GHz / 1000.0);
          printf("\n\n");
        }
        printf("[ISIP] Wait Loop time :%f us (%d trials, max %f us)\n",
               (double)eNB->isip_wait_loop.diff / eNB->isip_wait_loop.trials / cpu_freq_GHz / 1000.0,
               eNB->isip_wait_loop.trials,
               (double)eNB->isip_wait_loop.max / cpu_freq_GHz / 1000.0);
      }
  ...
  for (j = 0; j < ISIP_TURBO_THREAD_NUM; j++)
  {
    pthread_cond_signal(&eNB->isip_turbo_thread[j].cond_rx);
    pthread_join((eNB->isip_turbo_thread[j].pthread_rx), (void **)&status);
    pthread_mutex_destroy(&eNB->isip_turbo_thread[j].mutex_rx);
    pthread_cond_destroy(&eNB->isip_turbo_thread[j].cond_rx);
    printf("[ISIP][KILL] %d turbo thread\n", j);
  }
