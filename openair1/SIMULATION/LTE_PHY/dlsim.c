	...
	UE->dlsch[UE->current_thread_id[subframe]][eNB_id][1]->harq_ack[subframe].ack = 0;
	  ...
	  //PMI_FEEDBACK:
	    ...
	    proc_eNB->frame_tx++;
	      ...
	      LOG_M("txsigF0.m","txsF0", &eNB->common_vars.txdataF[0][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
	      if (eNB->frame_parms.nb_antenna_ports_eNB>1) 
		LOG_M("txsigF1.m","txsF1", &eNB->common_vars.txdataF[1][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
              if (transmission_mode == 7) {
                LOG_M("txsigF5.m","txsF5", &eNB->common_vars.txdataF[5][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
              } else if (transmission_mode == 8) {
                LOG_M("txsigF7.m","txsF7", &eNB->common_vars.txdataF[7][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
                LOG_M("txsigF8.m","txsF8", &eNB->common_vars.txdataF[8][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
	      }

	      LOG_M("txsigF_BF0.m","txsF_BF0", &ru->common.txdataF_BF[0][0],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
		...
		LOG_M("txsigF_BF1.m","txsF_BF1", &ru->common.txdataF_BF[1][0],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
		LOG_M("txsig1.m","txs1", &ru->common.txdata[1][subframe* eNB->frame_parms.samples_per_tti], eNB->frame_parms.samples_per_tti,1,1);
	      }
            }
	  }
