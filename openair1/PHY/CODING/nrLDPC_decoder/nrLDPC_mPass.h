...
static inline void nrLDPC_cn2bnProcBuf2_BG1(t_nrLDPC_lut* p_lut, t_nrLDPC_procBuf* p_procBuf, uint16_t Z) {

  int8_t* cnProcBufRes = p_procBuf->cnProcBufRes;
  int8_t* bnProcBuf    = p_procBuf->bnProcBuf;


  for (int i=0;i<p_lut->cn2bnProcBuf2_size/4;i+=3) {
    memcpy((void*)&bnProcBuf[p_lut->cn2bnProcBuf2[i+1]],
	   (void*)&cnProcBufRes[p_lut->cn2bnProcBuf2[i]],
	   p_lut->cn2bnProcBuf2[i+2]);
  }
}
/**
...
static inline void nrLDPC_bn2cnProcBuf2_BG1(t_nrLDPC_lut* p_lut, t_nrLDPC_procBuf* p_procBuf, uint16_t Z) {
  ...
  for (int i=0;i<p_lut->cn2bnProcBuf2_size/4;i+=3) {
    memcpy((void*)&cnProcBuf[p_lut->cn2bnProcBuf2[i]],
	   (void*)&bnProcBufRes[p_lut->cn2bnProcBuf2[i+1]],
	   p_lut->cn2bnProcBuf2[i+2]);
  }
}
