                                 ...
                                 unsigned char symbol)
{
  ...
  LTE_UE_DLSCH_t **dlsch_ue       = phy_vars_ue->dlsch[current_thread_id[Ns>>1]][eNB_id];
  ...
  rxdataF = phy_vars_ue->common_vars.common_vars_rx_data_per_thread[current_thread_id[Ns>>1]].rxdataF;

  dl_bf_ch_estimates = phy_vars_ue->pdsch_vars[current_thread_id[Ns>>1]][eNB_id]->dl_bf_ch_estimates;
 ...
 /* }
  ...
  for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
    ...
    rxF  = (short *)&rxdataF[aarx][pil_offset + frame_parms->first_carrier_offset + symbol*frame_parms->ofdm_symbol_size+subframe*(phy_vars_ue->frame_parms.ofdm_symbol_size*phy_vars_ue->frame_parms.symbols_per_tti)];
    ...
    if ((frame_parms->N_RB_DL&1) == 0) { // even number of RBs
      for (rb=0; rb<frame_parms->N_RB_DL; rb++) {
        ...
        if (rb==(frame_parms->N_RB_DL>>1)) {
          rxF       = (short *)&rxdataF[aarx][(1 + (symbol*(frame_parms->ofdm_symbol_size)))+subframe*(phy_vars_ue->frame_parms.ofdm_symbol_size*phy_vars_ue->frame_parms.symbols_per_tti)];
        }
      ...
      if (rb_alloc_ind==1) {
        if (pil_offset<2) {
          ...
          rxF   = (short *)&rxdataF[aarx][symbol*(frame_parms->ofdm_symbol_size)+subframe*(phy_vars_ue->frame_parms.ofdm_symbol_size*phy_vars_ue->frame_parms.symbols_per_tti)];
        ...
        rxF       = (short *)&rxdataF[aarx][pil_offset+((symbol*(frame_parms->ofdm_symbol_size)))+subframe*(phy_vars_ue->frame_parms.ofdm_symbol_size*phy_vars_ue->frame_parms.symbols_per_tti)];
      }
