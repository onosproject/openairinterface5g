              ...
              uint8_t cooperation_flag)
{
...
#ifdef CUDA
  unsigned short dftsizes_cu[33] = { 12,24,36,48,60,72,96,108,120,144,180,192,216,240,288,300,324,360,384,432,480,540,576,600,648,720,864,900,960,972,1080,1152,1200 };

  uint8_t b;
  ulsch_para[sched_subframe].u1 = frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.grouphop[(subframe<<1)];
  ulsch_para[sched_subframe].u2 = frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.grouphop[1+(subframe<<1)];
  ulsch_para[sched_subframe].v1 = frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.seqhop[(subframe<<1)];
  ulsch_para[sched_subframe].v2 = frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.seqhop[1+(subframe<<1)];
  ulsch_para[sched_subframe].Msc_RS = phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->nb_rb * 12;
  ulsch_para[sched_subframe].cyclic_shift1 = (frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.cyclicShift +
  ...
  ulsch_para[sched_subframe].cyclic_shift2 = (frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.cyclicShift +
                              phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->n_DMRS2 +
                              frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.nPRS[(subframe<<1)+1]) % 12;

  for (b=0; b<33; b++)
  {
    if (ulsch_para[sched_subframe].Msc_RS==dftsizes_cu[b])
	{
      ulsch_para[sched_subframe].Msc_RS_idx = b;
	  break;
	}
  }/*
  ...
  ulsch_extract_rb_cu(ulsch[UE_id]->harq_processes[harq_pid]->first_rb,
                      ulsch[UE_id]->harq_processes[harq_pid]->nb_rb,
                     (frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active),
                                      sched_subframe);
  estimation_cu(ulsch[UE_id]->harq_processes[harq_pid]->first_rb,
                ulsch[UE_id]->harq_processes[harq_pid]->nb_rb,
                (frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active),
                                sched_subframe);

  compensation_cu(ulsch[UE_id]->harq_processes[harq_pid]->first_rb,
                  ulsch[UE_id]->harq_processes[harq_pid]->nb_rb,
                  (frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active),
                                  Qm,
                                  sched_subframe);

  cudaMemcpyAsync(&eNB_pusch_vars->ulsch_power[0],
       ul_cu[sched_subframe].d_ulsch_power[0],
    sizeof(int),
    cudaMemcpyDeviceToHost,
    ul_cu[sched_subframe].stream_ul );
  cudaStreamSynchronize( ul_cu[sched_subframe].stream_ul);
  ...
  idft_cu(ulsch[UE_id]->harq_processes[harq_pid]->first_rb,
                  ulsch[UE_id]->harq_processes[harq_pid]->nb_rb,
                  (frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active),
                                  log2_maxh,
                                  sched_subframe);
  ...
  cudaMemcpyAsync(eNB_pusch_vars->rxdataF_comp[eNB_id][0],
                  ul_cu[sched_subframe].d_rxdata_comp_int[0],
                                  sizeof(int)* (frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active)* 300,
                                  cudaMemcpyDeviceToHost,
                                  ul_cu[sched_subframe].stream_ul );
  float time;
...
cudaEvent_t startEvent1, stopEvent1;
  ...
  cudaEventElapsedTime(&time1, startEvent1, stopEvent1);
  printf("[GPU] end of demodulation %f\n",time1);
