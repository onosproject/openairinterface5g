...
int create_gNB_tasks(uint32_t gnb_nb) {
  ...
  if (gnb_nb > 0) {
	...
	LOG_I(HW, "RCconfig_ric_agent\n");

#ifdef ENABLE_RIC_AGENT
  	RCconfig_ric_agent();
#endif

    LOG_I(NR_RRC,"Creating RIC_AGENT Task\n");
#ifdef ENABLE_RIC_AGENT
    if (itti_create_task (TASK_RIC_AGENT, ric_agent_task, NULL) < 0) {
      LOG_E(RIC_AGENT, "Create task for RIC_AGENT failed\n");
      return -1;
    }
#endif
  }
