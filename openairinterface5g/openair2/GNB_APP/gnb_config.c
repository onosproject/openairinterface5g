    ...
    if (strcmp(*(GNBParamList.paramarray[i][GNB_TRANSPORT_S_PREFERENCE_IDX].strptr), "f1") == 0) {
      ...
      if (macrlc_has_f1 == 0) {
        ...
        LOG_I(NR_RRC,"Setting node_type to ngran_gNB\n");
      	rrc->eth_params_s.my_addr                  = strdup(*(GNBParamList.paramarray[i][GNB_LOCAL_S_ADDRESS_IDX].strptr));
		rrc->node_name = strdup(*(GNBParamList.paramarray[0][GNB_GNB_NAME_IDX].strptr));
        LOG_I(GNB_APP,"gNB_name[%d] %s\n",k,rrc->node_name);
		LOG_I(NR_RRC, "ETH PARAMS: my_addr:%s\n",rrc->eth_params_s.my_addr);
#ifdef ENABLE_RIC_AGENT
  		nr_rrc_kpi_stats.nr_rrc_conn_estab_att_sum = 0;
  		nr_rrc_kpi_stats.nr_rrc_conn_estab_succ_sum = 0;
  		nr_rrc_kpi_stats.nr_rrc_conn_reestab_att_sum = 0;
  		nr_rrc_kpi_stats.nr_rrc_conn_mean = 0;
  		nr_rrc_kpi_stats.nr_rrc_conn_max = 0;
#endif       
      } else {
