            ...
            ptr++) 
    {
        ...
        if  (connUpdateIE->value.present == E2AP_E2connectionUpdate_IEs__value_PR_E2connectionUpdate_List) 
        {
            ...
            if (connUpdate_Item->value.present == E2AP_E2connectionUpdate_ItemIEs__value_PR_E2connectionUpdate_Item)
            {
                ...
                uint32_t decoded_ip_reordered=0;
				...
				decoded_ip_reordered = htonl(decoded_ip);

				RIC_AGENT_INFO("Reordered TNLAddress:%u\n",
                                decoded_ip_reordered);

                /* Binary to Decimal IPv4 Conversion */
                dec2strIpv4(decoded_ip_reordered, remote_ipv4);
                
                remote_port = decoded_port;

                /* CU & DU Differentiation */
                if ( (e2_conf[ranid]->e2node_type == E2NODE_TYPE_ENB_CU) || (e2_conf[ranid]->e2node_type == E2NODE_TYPE_GNB) )
                {
                ...
                {
                    //msg = itti_alloc_new_message(TASK_RIC_AGENT_DU, ranid, SCTP_NEW_ASSOCIATION_REQ);
                }
        ...
        uint32_t *outlen)
{
  ...
  {
    interval_sec = 2;
    //RIC_AGENT_INFO("Starting KPM Timer sec:%d us:%d\n",interval_sec, interval_us);
