...
static int ric_agent_connect(ranid_t ranid)
{
    ...
    msg = itti_alloc_new_message(TASK_RIC_AGENT, ranid, SCTP_NEW_ASSOCIATION_REQ);
        ...
        uint32_t assoc_id)
{
    ...
    msg = itti_alloc_new_message(TASK_RIC_AGENT, ric->ranid, SCTP_DATA_REQ);
...
static void ric_agent_disconnect(ric_agent_info_t *ric)
{
    ...
    msg = itti_alloc_new_message(TASK_RIC_AGENT, ric->ranid, SCTP_CLOSE_ASSOCIATION);
        ...
        uint32_t *assoc_id)
{
    ...
    */
    ...
    if (ric == NULL) {
        RIC_AGENT_ERROR("[%s]: ric agent info not found %ld\n", __func__, instance);
        return -1;
    }
...
void *ric_agent_task(void *args)
{
    ...
    while (1) {
        ...
        if (outlen) {
            instance_t instance = ITTI_MSG_ORIGIN_INSTANCE(msg);
...
void RCconfig_ric_agent(void) 
{
	...
	while(RC.nrrrc[0]->node_name == NULL)
	{
	    printf("RC.nrrrc:0x%x, Sleeping for 1 sec\n", RC.nrrrc);
		sleep(1);
	}
    ...
    else if (NODE_IS_DU(RC.nrrrc[0]->node_type))
    {
...
*/
        ...
        snprintf(buf, sizeof(buf), "%s.[%u].RIC", GNB_CONFIG_STRING_GNB_LIST, i);
                ...
                && strcmp(*ric_params[RIC_CONFIG_IDX_ENABLED].strptr, "yes") == 0) 
        {
            ...
            if (NODE_IS_MONOLITHIC(RC.nrrrc[i]->node_type))
            {
				...
				RIC_AGENT_INFO("RIC AGENT for MONOLITHIC gNB configured\n");
            } 
			...
			RIC_AGENT_INFO("E2 CONFIG: NodeName:%s CID:%u mcc:%u mnc:%u mncLen:%u\n",
							e2_conf[i]->node_name, e2_conf[i]->cell_identity, e2_conf[i]->mcc,
							e2_conf[i]->mnc, e2_conf[i]->mnc_digit_length);
			...
			RIC_AGENT_INFO("E2 CONFIG: Remote IP:%s Remote Port:%u e2node_typ:%u\n", e2_conf[i]->remote_ipv4_addr, 
							e2_conf[i]->remote_port, e2_conf[i]->e2node_type);
        }
    }
}
