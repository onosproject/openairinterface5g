...
extern gNB_RRC_KPI_STATS    nr_rrc_kpi_stats;
...
bool action_def_missing = 0;//FALSE;
...
kmp_meas_info_t e2sm_kpm_meas_info[MAX_KPM_MEAS] = {
                                            {1, "RRC.ConnEstabAtt.sum", 0, 0},//FALSE},
                                            {2, "RRC.ConnEstabSucc.sum", 0, 0},//FALSE},
                                            {3, "RRC.ConnReEstabAtt.sum", 0, 0},//FALSE},
                                            {4, "RRC.ConnMean", 0, 0},//FALSE},
                                            {5, "RRC.ConnMax", 0, 0}//FALSE}
                                        };
...
int e2sm_kpm_init(void)
{
    ...
    *ranFuncInst = 0;
    ...
    /* Fetching PLMN ID*/
    for (i = 0; i < RC.nb_nr_inst; ++i) { //is there a better way to fetch RANID,otherwise PLMNID of first intance will get populated ?
...

{
    ...
    {
        if (e2sm_kpm_meas_info[i].subscription_status == 1)//TRUE)
        {
            ...
            switch(e2sm_kpm_meas_info[i].meas_type_id)
            {
                case 1:/*RRC.ConnEstabAtt.sum*/
                    g_indMsgMeasRecItemArr[g_granularityIndx][j]->choice.integer = 
                                                                        nr_rrc_kpi_stats.nr_rrc_conn_estab_att_sum;
                    break;
                case 2:/*RRC.ConnEstabSucc.sum*/
                    g_indMsgMeasRecItemArr[g_granularityIndx][j]->choice.integer = 
                                                                        nr_rrc_kpi_stats.nr_rrc_conn_estab_succ_sum;
                    break;
                case 3:/*RRC.ConnReEstabAtt.sum*/
                    g_indMsgMeasRecItemArr[g_granularityIndx][j]->choice.integer = 
                                                                        nr_rrc_kpi_stats.nr_rrc_conn_reestab_att_sum;
                    break;
                case 4:/*RRC.ConnMean*/
                    g_indMsgMeasRecItemArr[g_granularityIndx][j]->choice.integer =
						   													nr_rrc_kpi_stats.nr_rrc_conn_max;	
                                                                            //f1ap_cu_inst[ric->ranid].num_ues;
                    break;
                case 5:/*RRC.ConnMax*/
                    g_indMsgMeasRecItemArr[g_granularityIndx][j]->choice.integer = nr_rrc_kpi_stats.nr_rrc_conn_max;
                    break;
                default:
                    break;
            }
    ...
    //RIC_AGENT_INFO("+++  Granularity Period expired, timer_id %ld function_id %ld g_granularityIndx:%d +++ \n",
    //               timer_id, function_id, g_granularityIndx);
    *outbuf = NULL;
    *outlen = 0;
    return 0;
}
                                          ...
                                          ric_agent_info_t *ric)
{
    ...
    for (i = 0; i < MAX_KPM_MEAS; i++)
    {
        e2sm_kpm_meas_info[i].subscription_status = 0;//FALSE;
    }
    ...
    if (def_size == 0)
    {
        ...
        for (i = 0; i < MAX_KPM_MEAS; i++)
        {
            ...
            e2sm_kpm_meas_info[i].subscription_status = 1;//TRUE;
            g_indMsgMeasInfoCnt++;
        }
        *g_granulPeriod = 10; //Hack
        ...
        action_def_missing = 1;//TRUE; /* Granularity Timer will not start */
        return 0;
    }
                            ...
                            E2SM_KPM_E2SM_KPMv2_ActionDefinition__actionDefinition_formats_PR_actionDefinition_Format1)
    {
        ...
        *g_granulPeriod = actionDefFormat1->granulPeriod;
        ...
        for (i=0; i < actionDefFormat1->measInfoList.list.count; i++)
        {
            ...
            if (actionDefMeasInfoItem->measType.present == E2SM_KPM_MeasurementType_KPMv2_PR_measName)
            {
                ...
                if ( ( (localMeasID > 0) &&
                     ...
                     (e2sm_kpm_meas_info[localMeasID-1].subscription_status == 0) )//FALSE) ) /*Avoid subscribing duplicate */
                {
                    ...
                    e2sm_kpm_meas_info[localMeasID-1].subscription_status = 1;//TRUE;
                    g_indMsgMeasInfoCnt++;
                }
                else
                {
		...
		//RIC_AGENT_INFO("Starting Granularity Period Timer us:%d ms:%d sec:%d\n",
		//				gp_interval_us, gp_interval_ms, gp_interval_sec);
...
encode_kpm_Indication_Msg(ric_agent_info_t* ric, ric_subscription_t *rs)
{
        ...
        for (i = 0; i < MAX_KPM_MEAS; i++)
        {
            ...
            switch(i)
            {
                case 0:/*RRC.ConnEstabAtt.sum*/
                    g_indMsgMeasRecItemArr[0][i]->choice.integer = nr_rrc_kpi_stats.nr_rrc_conn_estab_att_sum;
                    break;
                case 1:/*RRC.ConnEstabSucc.sum*/
                    g_indMsgMeasRecItemArr[0][i]->choice.integer = nr_rrc_kpi_stats.nr_rrc_conn_estab_succ_sum; 
                    break;
                case 2:/*RRC.ConnReEstabAtt.sum*/
                    g_indMsgMeasRecItemArr[0][i]->choice.integer = nr_rrc_kpi_stats.nr_rrc_conn_reestab_att_sum;
                    break;
                case 3:/*RRC.ConnMean*/
                    g_indMsgMeasRecItemArr[0][i]->choice.integer = f1ap_cu_inst[ric->ranid].num_ues;
                    break;
                case 4:/*RRC.ConnMax*/
                    g_indMsgMeasRecItemArr[0][i]->choice.integer = nr_rrc_kpi_stats.nr_rrc_conn_max;
                    break;
    ...
    //RIC_AGENT_INFO("Granularity Idx=:%d g_indMsgMeasInfoCnt:%d\n",g_granularityIndx, g_indMsgMeasInfoCnt);
...
void encode_e2sm_kpm_indication_header(ranid_t ranid, E2SM_KPM_E2SM_KPMv2_IndicationHeader_t *ihead) 
{
    ...
    if ( (node_type == E2NODE_TYPE_ENB_CU) || (node_type == E2NODE_TYPE_GNB) ) 
    {
        MCC_MNC_TO_PLMNID(
                e2_conf[ranid]->mcc,
                e2_conf[ranid]->mnc,
