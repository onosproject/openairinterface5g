...
rrc_eNB_process_RRCConnectionSetupComplete(
  ...
  LTE_RRCConnectionSetupComplete_r8_IEs_t * LTE_RRCConnectionSetupComplete
)
//-----------------------------------------------------------------------------
{
  LOG_I(RRC,
...
#ifdef RRC_MSG_PRINT
  LOG_F(RRC,"[MSG] LTE_RRCConnectionReject\n");
...
#ifdef RRC_MSG_PRINT
  LOG_F(RRC,"[MSG] LTE_RRCConnectionReestablishment\n");
...
#ifdef RRC_MSG_PRINT
  LOG_F(RRC,"[MSG] LTE_RRCConnectionReestablishmentReject\n");
...
//-----------------------------------------------------------------------------
{
  ...
  *(DRB_config->logicalChannelIdentity) = (long)3; // value : x+2
  ...
  DRB_rlc_config->present = LTE_RLC_Config_PR_am;
  DRB_rlc_config->choice.am.ul_AM_RLC.t_PollRetransmit = LTE_T_PollRetransmit_ms50;
  DRB_rlc_config->choice.am.ul_AM_RLC.pollPDU =  LTE_ LTE_PollPDU_p16;
  DRB_rlc_config->choice.am.ul_AM_RLC.pollByte = LTE_PollByte_kBinfinity;
  DRB_rlc_config->choice.am.ul_AM_RLC.maxRetxThreshold = LTE_UL_AM_RLC__maxRetxThreshold_t8;
  DRB_rlc_config->choice.am.dl_AM_RLC.t_Reordering = LTE_T_Reordering_ms35;
  DRB_rlc_config->choice.am.dl_AM_RLC.t_StatusProhibit =  LTE_ LTE_T_StatusProhibit_ms25;
#else
  DRB_rlc_config->present = LTE_RLC_Config_PR_um_Bi_Directional;
  DRB_rlc_config->choice.um_Bi_Directional.ul_UM_RLC.sn_FieldLength = LTE_SN_FieldLength_size10;
  DRB_rlc_config->choice.um_Bi_Directional.dl_UM_RLC.sn_FieldLength = LTE_SN_FieldLength_size10;
...
//              ue_context_p = rrc_eNB_get_next_free_ue_context(ctxt_pP, NOT_A_RANDOM_UE_IDENTITY);
	    ...
	    MSC_LOG_RX_MESSAGE(
			       ...
			       Srb_info->Rx_buffer.Payload,
			       dec_rval.consumed,
			       MSC_AS_TIME_FMT" LTE_RRCConnectionRequest UE %x size %u (s-TMSI mmec %u m_TMSI %u random UE id (0x%" PRIx64 ")",
			       MSC_AS_TIME_ARGS(ctxt_pP),
			       ue_context_p->ue_context.rnti,
			       dec_rval.consumed,
			       ue_context_p->ue_context.Initialue_identity_s_TMSI.mme_code,
	    ...
	    LOG_E(RRC,
		  PROTOCOL_RRC_CTXT_UE_FMT" LTE_RRCConnectionRequest without random UE identity or S-TMSI not supported, let's reject the UE\n",
		  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
	...
	/*FK: left the condition as is for the case MME is used (S1 mode) but setting  dedicated_DRB = 1 otherwise (noS1 mode) so that no second RRCReconfiguration message activationg more DRB is sent as this causes problems with the nasmesh driver.*/
	  ...
	  LOG_I(RRC,
		...
		PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),ul_dcch_msg->message.choice.c1.choice.rrcConnectionReconfigurationComplete.rrc_TransactionIdentifier);
          ...
          if(UE_id == -1){
            LOG_E(RRC,
                  PROTOCOL_RRC_CTXT_UE_FMT" LTE_RRCConnectionReconfigurationComplete without rnti %x, fault\n",
                  PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),ctxt_pP->rnti);
            break;
          }
