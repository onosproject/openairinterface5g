...
/* must be called with lock acquired */
void rlc_mbms_add_drb_rlc_entity(rlc_ue_t *ue, int drb_id, rlc_entity_t *entity)
{
  if (drb_id <= 5) {
    LOG_E(RLC, "%s:%d:%s: fatal, bad drb id\n", __FILE__, __LINE__, __FUNCTION__);
    exit(1);
  ...
  ue->drb[drb_id] = entity;
  LOG_E(RLC,"rlc_mbms_add_drb_rlc_entity ue->drb[%d]=%p\n",drb_id,ue->drb[drb_id]);
}
