    ...
    for (eNB_id=0; eNB_id<number_of_cards; eNB_id++) {
        ...
        for (UE_id=UE_info->list.head; UE_id>=0; UE_id=UE_info->list.next[UE_id]) {
            for (i=0; i<UE_info->numactiveCCs[UE_id]; i++) {
                ...
                UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_bitrate=(((UE_info->eNB_UE_stats[CC_id][UE_id].TBS[TB1]+UE_info->eNB_UE_stats[CC_id][UE_id].TBS[TB2])*8)/((eNB->frame + 1)*10));
                UE_info->eNB_UE_stats[CC_id][UE_id].total_dlsch_bitrate= ((UE_info->eNB_UE_stats[CC_id][UE_id].total_pdu_bytes*8)/((eNB->frame + 1)*10));
                UE_info->eNB_UE_stats[CC_id][UE_id].total_overhead_bytes+=  UE_info->eNB_UE_stats[CC_id][UE_id].overhead_bytes[TB1]+UE_info->eNB_UE_stats[CC_id][UE_id].overhead_bytes[TB2];
                ...
                len += sprintf(&buffer[len],"[MAC] UE %d (DLSCH),status %s, RNTI %x : CQI %d, MCS1 %d, MCS2 %d, RB (tx %d, retx %d, total %d), ncce (tx %d, retx %d) \n",
                               ...
                               UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB2],
                ...
                len += sprintf(&buffer[len],
                               ...
                               UE_info->eNB_UE_stats[CC_id][UE_id].overhead_bytes[TB1],
