...
#warning RK->CR This old API call has to be revisited for FAPI, or logic must be changed
      ...
      */
      ...
      if (get_tmode(module_idP,CC_id,UE_id)==3)
	eNB_UE_stats->dlsch_mcs2 = cqi_to_mcs[ue_sched_ctl->dl_cqi[CC_id]]; //in TM3 there is only one CQI value
      /*
	...
	TBS = get_TBS_DL(UE_list->UE_template[CC_id][UE_id].oldmcs1[harq_pid],nb_rb);
	  ...
	  */
	    ...
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.dci_format                  = get_tmode(module_idP,CC_id,UE_id)==3 ? NFAPI_DL_DCI_FORMAT_2A : NFAPI_DL_DCI_FORMAT_1;
	    ...
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_1        = UE_list->UE_template[CC_id][UE_id].oldNDI[1][harq_pid];
	    ...
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2        = UE_list->UE_template[CC_id][UE_id].oldNDI[1][harq_pid];
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_2                       = UE_list->UE_template[CC_id][UE_id].oldmcs2[harq_pid];
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_2        = round&3;
	    ...
	    if (!CCE_allocation_infeasible(module_idP,CC_id,1,subframeP,
	      ...
	      fill_nfapi_dlsch_config(eNB,dl_req,
			     ...
			     //			     uint8_t codebook_index,
				      ...
				      0, // ngap
				      0, // nprb
				      get_tmode(module_idP,CC_id,UE_id), // transmission mode
	...
	TBS = get_TBS_DL(mcs,nb_available_rb);
        ...
        if ((tb==0) && (ue_sched_ctl->ta_timer == 0)) {
	...
	if ((sdu_length_total + header_len_dcch + header_len_dtch )> 0) {
	  ...
	  //#endif
          ...
          offset = generate_dlsch_header((unsigned char*)&UE_list->DLSCH_pdu[CC_id][tb][UE_id].payload[0][0],
                                         ...
                                         sdu_lcids,
                                         255,                                   // no drx
                                         (tb==0 ? ta_update : 0), // timing advance
	...
	if (UE_list->UE_template[CC_id][UE_id].TB_active[0][harq_pid] ||
	  ...
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_1        = ndi1;
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_1                       = mcs1;
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_1        = rv1;
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2        = ndi2;
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_2                       = mcs2;
	  dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_2        = rv2;
	  if (cc[CC_id].tdd_Config != NULL) { //TDD
	    dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.downlink_assignment_index = (UE_list->UE_template[CC_id][UE_id].DAI-1)&3;
	  ...
	  if (!CCE_allocation_infeasible(module_idP,CC_id,1,subframeP,dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.aggregation_level,rnti)) {
	    ...
	    LOG_D(MAC,"CC_id %d Frame %d, subframeP %d: Toggling Format1 NDI for UE %d (rnti %x/%d) oldNDI %d\n",
		  ...
		  rnti,harq_pid,UE_list->UE_template[CC_id][UE_id].oldNDI[0][harq_pid]);
	    
	    UE_list->UE_template[CC_id][UE_id].oldNDI[0][harq_pid]=1-UE_list->UE_template[CC_id][UE_id].oldNDI[1][harq_pid];
	    UE_list->UE_template[CC_id][UE_id].oldNDI[1][harq_pid]=1-UE_list->UE_template[CC_id][UE_id].oldNDI[1][harq_pid];
	    UE_list->UE_template[CC_id][UE_id].oldmcs1[harq_pid] = mcs1;
	    UE_list->UE_template[CC_id][UE_id].oldmcs2[harq_pid] = mcs2;
