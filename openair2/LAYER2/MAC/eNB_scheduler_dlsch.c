...
//------------------------------------------------------------------------------
{
  ...
  for (CC_id = 0, eNB_stats = &eNB->eNB_stats[0]; CC_id < nb_mac_CC; CC_id++, eNB_stats++) {
    ...
    if (N_RB_DL[CC_id] == -1) {
      LOG_E(MAC, "schedule_ue_spec:to_prb failed\n");
      return;
    }
    ...
    for (UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
      ...
      if (continue_flag != 1) {
        switch (get_tmode(module_idP,
          ...
          default:
            LOG_E(MAC, "Unsupported transmission mode %d\n", get_tmode(module_idP, CC_id, UE_id));
            return;
        }
      }
      ...
      if (round_DL != 8) {
        ...
        if (ta_len + sdu_length_total + header_length_total > 0) {
          ...
          offset = generate_dlsch_header((unsigned char *) UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1],
                                         ...
                                         post_padding);
          if (offset < 0) {
			LOG_E(MAC, "schedule_ue_spec:generate_dlsch_header failed\n");
			return;
		  }
          ...
          T(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
            ...
            T_INT(subframeP),
            T_INT(harq_pid),
            T_BUFFER(dlsch_pdu->payload[0][TB1],
                     TBS));
...
//------------------------------------------------------------------------------
{
  ...
  for (CC_id = 0; CC_id < nb_mac_CC; CC_id++) {
    ...
    if (N_RBG[CC_id] < 0) {
      LOG_E(MAC, "dlsch_scheduler_interslice_multiplexing:to_rbg failed\n");
	  return;
    }
    ...
    for (i = 0; i < sli->n_dl; ++i) {
      ...
      for (UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
        ...
        tm = get_tmode(Mod_id,
                       ...
                       UE_id);
        if (tm == 0) {
          LOG_E(MAC, "dlsch_scheduler_interslice_multiplexing:get_tmode failed\n");
		  return;
        }
    ...
    if (subframeP == 5) { // MPDCCH
      ...
      if (round_DL == 8) {
        ...
        if ((sdu_length_total + header_len_dcch + header_len_dtch) > 0) {
          ...
          /* Cycle through SDUs and place in dlsch_buffer */
          memcpy(&UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1][offset], dlsch_buffer, sdu_length_total);
          ...
          if (opt_enabled == 1) {
            trace_pdu(1,
                      (uint8_t *)UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1],
                      TBS,
                      module_idP,
                      3,
          ...
          T(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
            ...
            T_INT(subframeP),
            T_INT(harq_pid),
            T_BUFFER(UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1], TBS));
          /* Do PUCCH power control */
          /* This is the snr */
      ...
      T(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
        ...
        T_BUFFER (&mac->UE_list.DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1], TX_req->pdu_length));

      if (opt_enabled == 1) {
        trace_pdu(1,
                  (uint8_t *) mac->UE_list.DLSCH_pdu[CC_id][0][(unsigned char) UE_id].payload[0][TB1],
                  TX_req->pdu_length,
                  UE_id,
                  3,
                  rnti,
...
//------------------------------------------------------------------------------
{
  ...
  if (UE_id != -1) {
    LOG_D(MAC, "[eNB %d] Frame %d:  CC_id %d Get DLSCH sdu for rnti %x => UE_id %d\n",
          ...
          rntiP,
          UE_id);
    return ((unsigned char *) &eNB->UE_list.DLSCH_pdu[CC_id][TBindex][UE_id].payload[0][TB1]);
  }
