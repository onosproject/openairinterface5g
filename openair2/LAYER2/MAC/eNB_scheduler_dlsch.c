...
static void basic_dl(int module_idP, int frameP, int subframeP)
{
  if (module_idP !=  0) abort();
...
printf("DL remove rnti %d\n", list[i]);
  ...
  for (i = 0; i < list_size; i++) {
    ...
    if (eNB_UE_stats == NULL) abort();
    /* dl_pow_offset is set in dlsch_scheduler_pre_processor_reset */
  ...
  for (i = 0; i < list_size; i++) {
    ...
    if (CCE_allocation_infeasible(module_idP,CC_id,0,subframeP,aggregation,rnti)) { printf("CCE allocation impossible\n"); return; continue; }

      nb_available_rb = total_nb_available_rb[CC_id];
    ...
    if (round == 0) continue;
     ...
     if (nb_rb > nb_available_rb) { printf("cannot retransmit, do nothing\n"); return; continue; }
...
UE_list->next[UE_id] = -1;
  ...
  for (i = 0; i < list_size; i++) {
    rnti = list[i];
    UE_id = get_ue_id(rnti);
if (subframeP == 6 || subframeP == 7 || subframeP == 8) continue;
if (subframeP == 9 || subframeP == 0 || subframeP == 1) continue;
//if (UE_id + 1 /*+5*/ != subframeP) continue;
    eNB_UE_stats = mac_xface->get_eNB_UE_stats(module_idP,CC_id,rnti);
    ue_sched_ctl = &UE_list->UE_sched_ctrl[UE_id];
    add_ue_dlsch_info(module_idP, CC_id, UE_id, subframeP, S_DL_NONE);
    if (eNB_UE_stats == NULL) abort();
    ...
    if (CCE_allocation_infeasible(module_idP,CC_id,0,subframeP,aggregation,rnti)) { printf("CCE allocation (new) impossible\n"); return; continue; }

    if (frame_parms[CC_id]->N_RB_DL == 25)
      nb_available_rb = 9; //total_nb_available_rb[CC_id];
    if (frame_parms[CC_id]->N_RB_DL == 50)
      nb_available_rb = 15;
    else abort();
    ...
    if (round != 0) { abort(); continue; }

    /* TODO: fake alloc */
if (frame_parms[CC_id]->N_RB_DL == 25)
{
    ue_sched_ctl->rballoc_sub_UE[CC_id][12] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][11] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][10] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][9] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][8] = 1;
}
else if (frame_parms[CC_id]->N_RB_DL == 50)
{
    ue_sched_ctl->rballoc_sub_UE[CC_id][15] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][14] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][13] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][12] = 1;
    ue_sched_ctl->rballoc_sub_UE[CC_id][11] = 1;
}
else abort();
	...
	if ((sdu_length_total + header_len_dcch + header_len_dtch )> 0) {
	  ...
	  if (header_len_dtch==0) {
	    ...
	    header_len_dtch = (header_len_dtch > 0) ? header_len_dtch - header_len_dtch_last  :header_len_dtch;     // remove length field for the last SDU
          ...
          if(0 && nb_rb == ue_sched_ctl->pre_nb_available_rbs[CC_id]) {
...
if (tpc != 1)
if (frameP == 0)
	      printf("[eNB %d] DLSCH scheduler: PUCCH power update rnti %d frame %d, subframe %d, harq_pid %d, tpc %d, accumulated %d, normalized/target rx power %d/%d\n",
		    module_idP,rnti,frameP, subframeP,harq_pid,tpc,
		    tpc_accumulated,normalized_rx_power,target_rx_power);
...
{
#ifdef BASIC_SCHED_HACK
  return basic_dl(module_idP, frameP, subframeP);
#endif
