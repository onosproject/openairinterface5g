...
for(int x=0; x<20; x++) {  // two frames for initial sync
	...
	if (toTrash)
	  rxp[i]=malloc16(UE->frame_parms.samples_per_slot*4);
	else
	  rxp[i] = ((void *)&UE->common_vars.rxdata[i][0]) +
	    4*(x*UE->frame_parms.slots_per_subframe+slot)*UE->frame_parms.samples_per_slot;
      }
      
      AssertFatal( UE->frame_parms.samples_per_slot ==
	...
	dummyWrite(UE,*timestamp, UE->frame_parms.samples_per_slot);
      if (toTrash)
	for (int i=0; i<UE->frame_parms.nb_antennas_rx; i++)
	  free(rxp[i]);
    }
...
void *UE_thread(void *arg) {
  ...
  while (!oai_exit) {
    ...
    if (slot_nr==18 || IS_SOFTMODEM_RFSIM)
      AssertFatal(writeBlockSize ==
                  UE->rfdevice.trx_write_func(&UE->rfdevice,
                     timestamp+
