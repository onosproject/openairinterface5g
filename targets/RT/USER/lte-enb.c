...
static inline int rxtx(PHY_VARS_eNB *eNB, eNB_rxtx_proc_t *proc, char *thread_name)
{
  ...
  if (eNB->proc_tx)
    eNB->proc_tx(eNB, proc, no_relay, NULL);

  if (release_thread(&proc->mutex_rxtx, &proc->instance_cnt_rxtx, thread_name) < 0)
    return (-1);
...
static void *eNB_thread_rxtx(void *param)
{
  ...
  while (!oai_exit)
  {
    ...
    if (wait_on_condition(&proc->mutex_rxtx, &proc->cond_rxtx, &proc->instance_cnt_rxtx, thread_name) < 0)
      break;
...
static void *eNB_thread_asynch_rxtx(void *param)
{
  ...
  while (!oai_exit)
  {
    ...
    if (eNB->fh_asynch)
      eNB->fh_asynch(eNB, &frame, &subframe);
    else
...
void rx_rf(PHY_VARS_eNB *eNB, int *frame, int *subframe)
{
  ...
  if (proc->subframe_rx > 5)
    proc->frame_tx = (proc->frame_tx + 1) & 1023;
...
void rx_fh_if4p5(PHY_VARS_eNB *eNB, int *frame, int *subframe)
{
  ...
  if (eNB->CC_id == 1)
    LOG_I(PHY, "rx_fh_if4p5: frame %d, subframe %d\n", *frame, *subframe);
  do
  { // Blocking, we need a timeout on this !!!!!!!!!!!!!!!!!!!!!!!
    ...
    else if (packet_type == IF4p5_PRACH)
    {
      ...
      if (eNB->do_prach)
        eNB->do_prach(eNB, f, sf);
    }

    if (eNB->CC_id == 1)
      LOG_I(PHY, "rx_fh_if4p5: symbol_mask[%d] %x\n", *subframe, proc->symbol_mask[*subframe]);
  ...
  if (eNB->CC_id == 1)
    LOG_I(PHY, "Clearing symbol_mask[%d]\n", *subframe);
...
static void *eNB_thread_FH(void *param)
{
  ...
  while (!oai_exit)
  {
    ...
    if (eNB->rx_fh)
      eNB->rx_fh(eNB, &frame, &subframe);
    else
      AssertFatal(1 == 0, "No fronthaul interface : eNB->node_function %d", eNB->node_function);
...
static void *eNB_thread_single(void *param)
{
  ...
  while (!oai_exit)
  {
    ...
    if (eNB->CC_id != 0)
      proc_rxtx->frame_tx = (proc_rxtx->frame_tx + proc->frame_offset) & 1023;

    // At this point, all information for subframe has been received on FH interface
    // If this proc is to provide synchronization, do so
    wakeup_slaves(proc);

    if (rxtx(eNB, proc_rxtx, "eNB_thread_single") < 0)
      break;
  }
...
static void *isip_turbo_proc(void *param)
{
  ...
  printf("[ISIP][SCHED][eNB] Turbo thread %d started on CPU %d TID %ld\n", turbo_pram->thread_id, sched_getcpu(), gettid());
  while (!oai_exit)
  {
  ...
  // pthread_mutex_unlock(&eNB->isip_turbo_thread[turbo_pram->thread_id].mutex_rx);
...
void kill_eNB_proc(int inst)
{
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++)
  {
    ...
    // destory ISIP turbo thread
    for (i = 0; i < ISIP_TURBO_THREAD_NUM; j++)
    {
      pthread_cond_signal(&eNB->isip_turbo_thread[i].cond_rx);
      pthread_join((eNB->isip_turbo_thread[i].pthread_rx), (void **)&status);
      pthread_mutex_destroy(&eNB->isip_turbo_thread[i].mutex_rx);
      pthread_cond_destroy(&eNB->isip_turbo_thread[i].cond_rx);
