...
%rf_mode  = (RXEN + TXEN + TXLPFNORM + TXLPFEN + TXLPF25 + RXLPFNORM + RXLPFEN + RXLPF25 + LNA2ON + LNAMed + RFBBNORM)*[1 0 0 0];
%rf_mode  = (TXEN + TXLPFNORM + TXLPFEN + TXLPF25 + RXLPFNORM + RXLPFEN + RXLPF25 + LNA2ON + LNAMax + RFBBNORM)*[1 0 0 0];
...
%freq_rx   = 2542000000*[1 0 0 0];
freq_rx   = 1900000000*[1 0 0 0];
...
%DUPLEXMODE_FDD=0
%TXRXSWITCH_LSB=0
tdd_config = 32 + DUPLEXMODE_TDD + TXRXSWITCH_LSB;
...
oarf_config_exmimo(card, freq_rx, freq_tx, tdd_config, dual_tx, rx_gain, tx_gain, eNBflag, rf_mode, rf_rxdc, rf_local, rf_vcocal, rffe_rxg_low, rffe_rxg_final, rffe_band, autocal, resampling_factor)
...
case 10
	s(:,1) = amp * ones(1,length)*2;
%	s(1:length/2+120,1) = 0 * ones(1,length/2+120);
	s(1:length/4,1) = amp * ones(1,length/4)*2 +1;

case 11

	s(:,1) = amp * ones(1,length)*2;
%	s(1:length/2+120,1) = 0 * ones(1,length/2+120);
	s(1:length/4,1) = 0 * ones(1,length/4) +1;

	oarf_send_frame(card, s, 16);

	sleep(1)

	s(:,1) = 0 * ones(1,length)*2 +1;	% Tx I/Q set to 0 AND RX mode on all frame

%	s(:,1) = repmat(1+1j,length/4,1); %make sure LSB is 1 (switch=rx)
