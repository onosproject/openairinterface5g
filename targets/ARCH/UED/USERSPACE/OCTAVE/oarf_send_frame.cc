...
DEFUN_DLD (oarf_send_frame, args, nargout,"Send frame")
{
	...
	printf("card %d: openair0_num_detected_cards=%d\n\n\n", card, openair0_num_detected_cards);
	...
	printf("numcols=%d, openair0_num_antennas[%d]=%d, max_antenna=%d\n\n\n", numcols, card, openair0_num_antennas[card], MAX_ANTENNAS);
...
//					printf("%d: %d,%d\n",i,(short)real(dx(i,aa)),(short)imag(dx(i,aa)));
				...
				if( ((unsigned int*)openair0_exmimo_pci[card].dac_head[aa])[i] )
					printf("%d: %d,%d - 0x%x\n",i,(short)real(dx(i,aa)),(short)imag(dx(i,aa)), ((unsigned long*)openair0_exmimo_pci[card].dac_head[aa])[i]);
            }
        }
        else if (nbits==8)
        {
