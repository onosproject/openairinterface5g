...
int exmimo_assign_shm_vars(int card_id)
{
	...
	{
		p_exmimo_pci_phys[card_id]	= (exmimo_pci_interface_bot_t *) bigshm_assign( card_id, sizeof(exmimo_pci_interface_bot_t), &pphys_exmimo_pci_phys[card_id]);
		...
		exmimo_pci_kvirt[card_id].firmware_block_ptr = (char *) bigshm_assign( card_id, MAX_FIRMWARE_BLOCK_SIZE_B, (dma_addr_t*)&(p_exmimo_pci_phys[card_id]->firmware_block_ptr));
		...
		exmimo_pci_kvirt[card_id].printk_buffer_ptr = (char *) bigshm_assign( card_id, MAX_PRINTK_BUFFER_B, (dma_addr_t*)&(p_exmimo_pci_phys[card_id]->printk_buffer_ptr));
...
int exmimo_send_pccmd(int card_id, unsigned int cmd)
{
	...
	val = ioread32(bar[card_id]+PCIE_CONTROL1);
	printk("Sending command to ExpressMIMO (card %d) : prev_ctrl1_val=%.4x cmd=%.4x\n",card_id, val, cmd);
...
///	printk("Readback of control0 %x\n",ioread32(bar[0]+PCIE_CONTROL0));
	...
	// SYRTEM chnaged from 100*100ms to 10000*1ms
