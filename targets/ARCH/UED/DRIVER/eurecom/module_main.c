...
static int __init openair_init_module(void)
{
	...
	if(pdev[card])
	{
		...
		printk("[openair][INIT_MODULE][INFO]: card %d: ExpressMIMO-%i (HW Rev %i), Bitstream: %s, SW/Protocol Revision: 0x%02X\n",
				card,
				exmimo_id_tmp[card].board_exmimoversion,
				exmimo_id_tmp[card].board_hwrev,
				((exmimo_id_tmp[card].board_vendor == EURECOM_VENDOR) ? "Eurecom" : "Telecom Paristech"),
				exmimo_id_tmp[card].board_swrev );
	...
	for (card = 0; card < number_of_cards; card++)
	{
		...
		if (irq != 0)
		{
			if ( (res = request_irq(irq, openair_irq_handler, IRQF_SHARED, "openair_rf", pdev[card])) == 0 )
			{
		...
		{
			if ( (res = request_irq(pdev[card]->irq, openair_irq_handler, IRQF_SHARED, "openair_rf", pdev[card])) == 0 )
			{
				openair_irq_enabled[card]	= 1;
			}
			else
